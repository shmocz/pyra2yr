    ...
    def unique_events(self):
        ...
        for l in self.lists:
            for k in ["out_list", "do_list", "megamission_list"]:
                all_events.extend((k, x.frame, x.timing, x) for x in getattr(l, k))
        ...
        for j in range(0, ry):
            place_query_grid.append(
                tuple2coord(
                    tuple(
                        x + y * 256
                        for x, y in zip(coords, (i - int(rx / 2), j - int(ry / 2), 0))
                    )
                )
            )
...
def read_file(p, mode="r", encoding="utf8"):
    with open(p, mode, encoding=encoding) as f:
        return f.read()


def write_file(p, data, mode="w", encoding="utf8"):
    with open(p, mode, encoding=encoding) as f:
        f.write(data)
