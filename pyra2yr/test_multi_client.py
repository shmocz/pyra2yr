...
class MultiTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def deploy_mcvs(self):
        for M in self.managers:
            o = next(M.state.query_objects(p=PT_MCV, h=M.state.current_player()))
            await M.M.deploy(o.o)
        # wait until deployed
        await self.managers[0].wait_state(
            lambda: len(list(self.managers[0].state.query_objects(p=PT_CONYARD)))
            == len(self.managers)
        )
        await self.managers[0].wait_state(
            lambda: all(
    ...
    async def check_engi_capture(
        ...
        self.assertNotEqual(target.get().pointer_house, engi.get().pointer_house)
        # Capture
    ...
    async def do_check_repair_building(self, M: MyManager, num_attackers=5):
        ...
        o_tesla = next(
            M.state.query_objects(p=PT_TESLA_REACTOR, h=M.state.current_player())
        )
        ...
        I = D.ind2sub(
            np.array(
                [
                    c.index
                    for c in D.m.cells
                    if o_mcv.get().pointer_self in [q.pointer_self for q in c.objects]
                ]
            )
        )
        # Get corners
